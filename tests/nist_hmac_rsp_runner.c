    ...
    const VectorCase *test_case)
{
    ...
    if (returned_bytes == 0 ||
        returned_bytes > HMAC_DRBG_MAX_BYTES_PER_REQUEST)
    {
        fprintf(
            stderr,
...
cleanup:
    ...
    if (ctx != NULL && HMACDRBG.uninstantiate != NULL)
    {
...
int main(int argc, char *argv[])
{
    ...
    {
        fprintf(stderr, "HMACDRBG descriptor is incomplete.\n");
        return EXIT_FAILURE;
    }
    ...
    printf("\nNIST HMAC_DRBG SHA-256 summary\n");
    printf("===============================\n");
    printf("Tested       : %zu\n", stats.tested);
    printf("Passed       : %zu\n", stats.passed);
    printf("Failed       : %zu\n", stats.failed);
    ...
    if (stats.tested == 0)
    {
        fprintf(
            stderr,
            "No SHA-256 HMAC_DRBG test cases were found.\n");
